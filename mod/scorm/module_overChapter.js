 ...
 */
/**
 * mod-scorm:module.js has been modified by below.
 * @copyright  2017 ‘—§î•ñŠwŒ¤‹†Š/National Institute of Informatics
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
if(nav_display==4){
	...
	if(node === null){
		node = scorm_launchablenext(scorm_current_node, true);
	}
            if (!node) {
                return;
            }
	...
	if(nextnode === null){
		nextnode = scorm_launchablenext(scorm_current_node, true);
	}
        ...
        var scorm_launchablenext = function(node, update_launch_sco) {
            if (node === false) {
                return scorm_tree_node.children[0];
            }
            if (typeof scoes_nav[launch_sco].nextscoid != 'undefined') {
		var next_launchable_scoid = scorm_search_next(launch_sco);
		node = scorm_tree_node.getNodeByAttribute('title', scoes_nav[next_launchable_scoid].url);
                if (update_launch_sco) {
                    launch_sco = next_launchable_scoid;
                }
                return node;
            }
            return null;
        };
	var scorm_search_next = function(tempscoid) {
		var nextscoid = scoes_nav[tempscoid].nextscoid;
		var launchValue = scoes_nav[nextscoid].launch;
		if (launchValue !== "") {
			return nextscoid;
		}
		return scorm_search_next(nextscoid);
	};
